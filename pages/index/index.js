...
Page(Object.assign({}, Zan.TopTips, Zan.NoticeBar, app.Methods, Modules.Methods, {
    data: Object.assign({}, app.Variables, Modules.Variables, {
        ...
        couponLogo: app.data.couponLogo,
    ...
    searchSubmit(e) {
        ...
        if (search == '') {
            ...
            wx.navigateTo({
                url: '/pages/product-list/product-list?search=' + search
            })
            this.clearSearch();
        }
    ...
    imageLoad(e) {
        ...
        var bannerSettings = this.data.bannerSettings;
        // 把每一张图片的高度记录到数组里
        if (bannerSettings[moduleIndex] == undefined) {
            bannerSettings[moduleIndex] = {};
        }
        if (bannerSettings[moduleIndex].heights == undefined) {
            bannerSettings[moduleIndex].heights = [];
        }
        bannerSettings[moduleIndex].heights[index] = imgHeight;
        bannerSettings[moduleIndex].current = 0;
        this.setData({
    ...
    swiperChange(e) {
        ...
        // 当前banner是视频时自动播放(首页模块会有较多视频所以不自动播放)
        /*if (type == 'video') {
            var id = type + '_' + moduleIndex + '_' + current,
                context = wx.createVideoContext(id);
            context.play();
        } else if (type == 'txvideo') {
            var id = banner[current].image,
    ...
    load() {
        var params = {};
        if (this.data.contentType == 'module' && this.options.template_id) {
            params.template_id = this.options.template_id;
        }
    ...
    onLoad(options) {
        ...
        /* W2W Extension, Name: w2w-advanced-coupon, Code: onLoadCouponPopup */
        if (this.options.coupon) {
            var code = this.options.coupon;
            delete this.options.coupon;
            this.receiveCouponRequest(code, data => {
                this.setData({
                    couponPopup: data
                }, () => {
    ...
    onPageScroll(e) {
        ...
        if (this.searchBgRestore) {
            if (scrollTop > this.searchBgRestoreHeight && this.data.modules[this.searchModuleIndex].bg != this.navBarBackgroundColor) {
                ...
                if (this.searchNavReverse) {
                    app.setNavStyle(this.navBarTheme);
                }
